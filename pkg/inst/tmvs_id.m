% -*- texinfo -*-
% @deftypefn {Function File} {@var{id} =} tmvs_id (@var{str})
%
% Parses the string @var{str} containing a record identifier and
% produces the identifying structure @var{id}.
...
% This is precomputed for better performance.
% any = '.{1,4}';
% space = '[\t ]+';
% number = '[0-9]+';
% pat = strcat( ...
...
% '(?<garbage>(?:', any, ')+?))?)$');
pat = '^(?<id>(?<testLab>KoeRak)(?<site>[A-Z])(?:(?<wall>S)|(?<floor>L)|(?<ceiling>K))(?<room1>[0-9]+)(?:[\t ]+Meta)?[\t ]+-[\t ]+(?:(?:(?<shortTemperature>T)|(?<shortRelativeHumidity>RH)|(?<shortAbsoluteHumidity>AH))(?<room2>[0-9]+)[\t ]+(?:(?:(?<centerFloor>L)|(?<centerCeiling>K))(?:at)?|(?<bottomCorner>A)|(?<topCorner>Y))(?<ordinal>[0-9]+)[\t ]+(?<position>[0-9]+)mm(?:[\t ]+(?<element>[0-9]+(?:[\t ]+)?(?:(?<mineralWool>villa)|(?<polystyrene>EPS)|(?<polyurethane>PUR))?))?|(?<magicNumber>lisa140))|(?:(?<autiolahti>Autiolah(?:ti|den))|(?<jyvaskyla>Jyv.{1,4}s?kyl.{1,4}n?))[\t ]+(?<weatherStation>s.{1,4}(?:.{1,4})?(?:[\t ]+)?(?:asema)?)[\t ]+-[\t ]+(?:(?:ilman|ulko|vallitseva|ymp.{1,4}rist.{1,4}n)(?:[\t ]+)?)*(?:(?<longTemperature>l.{1,4}mp.{1,4}(?:[\t ]+)?tila)|(?<longRelativeHumidity>(?:suhteellinen(?:[\t ]+)?)?kosteus(?:[\t ]+?prosentti)?)|(?<longAbsoluteHumidity>absoluuttinen(?:[\t ]+)?kosteus)|(?<longPressure>paine)|(?<longWindSpeed>tuul(?:i|en)(?:(?:[\t ]+)?nopeus)?)|(?<longPrecipitation>s(?:ade|ateen)(?:(?:[\t ]+)?m.{1,4}.{1,4}r.{1,4})?))(?:[\t ]+?arvo|mittaus)*(?:[\t ]+(?<garbage>(?:.{1,4})+?))?)$';
...
if ~isempty (nm.ordinal)
  ordinal = uint32 (str2double (nm.ordinal));
end
